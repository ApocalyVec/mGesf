...
class Control_tab(QWidget):
    ...
    def __init__(self, mmw_worker: MMW_worker, refresh_interval, *args, **kwargs):
        ...
        self.radar_connection_block = init_container(parent=self.radar_block,
                                                     label="Connection",
                                                     style="background-color: " + config.container_color + ";")
        self.radar_sensor_block = init_container(parent=self.radar_block,
                                                 label="Sensor",
                                                 style="background-color: " + config.container_color + ";")
        self.radar_runtime_view = self.init_spec_view(parent=self.radar_block,
                                                      label="Runtime",
                                                      graph=self.doppler_display)
        self.radar_record_checkbox = init_checkBox(parent=self.radar_block,
                                                   label="Record",
                                                   function=self.radar_clickBox)
        ...
        self.leap_runtime_view = self.init_spec_view(parent=self.leap_block,
                                                     label="Runtime")
        self.leap_record_checkbox = init_checkBox(parent=self.leap_block,
                                                  label="Record",
        ...
        self.UWB_runtime_view = self.init_spec_view(parent=self.UWB_block,
                                                    label="Runtime")
        self.UWB_record_checkbox = init_checkBox(parent=self.UWB_block,
                                                 label="Record",
                                                 function=self.UWB_clickBox)
        ...
        if self.will_recording_radar:
            if os.path.exists(data_path):
                ...
                if not self.is_recording_radar:
                    ...
                    self.record_btn.setText(config.record_btn_end_label)
                else:
                    self.is_recording_radar = False
                    self.record_btn.setText(config.record_btn_start_label)
