...
class LeapInterface:
    def __init__(self):
        ...
        prev_dir = os.getcwd()
        os.chdir(lib_path)
        self._leap = CDLL(lib_file)
        os.chdir(prev_dir)

        # set up function args
        self._leap.getInfo.argtypes = [POINTER(cLeapInfo)]
        self._leap.getFrame.argtypes = [POINTER(cLeapFrame)]
    ...
    def getLeapInfo(self):
        ...
        """
        info = cLeapInfo()
        self._leap.getInfo(info)

        return self.LeapInfo(service=info.service, connected=info.connected, has_focus=info.has_focus)
