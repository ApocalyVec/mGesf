...
void xt_config_build(void)
{
    ...
    for (uint32_t i = 0; i < sizeof(config_bindings)/sizeof(xt_config_binding_t); i++)
    {
        ...
        if (xt_config_find_section(config_bindings[i].id, &length, &offset) == XT_SUCCESS)
        {
...
int xt_get_default_radar_config(xt_radar_t * config)
{
    config->external_load_caps = XT_CONFIG_RADAR_EXT_LOAD_CAPS();
    config->center_frequency = XT_CONFIG_RADAR_CENTER_FREQ();
    config->tx_power = XT_CONFIG_RADAR_TX_POWER();
    config->prf_div = XT_CONFIG_RADAR_PRF_DIV();

    config->frame_area_offset = XT_CONFIG_RADAR_FRAME_AREA_OFFSET();
