...
typedef enum {
    ...
    XTIO_SERIAL_SDA = 84,   ///< ID for SDA pin
                            ///< This pin normally is used for serial communication
    
    XTIO_SERIAL_SCL = 85,   ///< ID for SCL pin
                            ///< This pin normally is used for serial communication

    XTIO_SERIAL_IRQ = 86,   ///< ID for IRQ pin
                            ///< This pin normally is used for serial communication
...
/**
 * @brief Controls whether or not to use external memory.
 * Requires a system reset to take effect.
 *
...
/**
 * @brief Checks if external memory is enabled
 *
 * @return True if memory is enabled, false otherwise
 */
...
/**
 * @brief Checks if fast memory is enabled
 *
 * @return True if memory is enabled, false otherwise
 */
...
/**
 * @brief Gets the submode of the module for certification
 *
 * @return Operation mode as defined in @ref xt_opmode_t
 */
 ...
 */
uint32_t xtio_flash_erase(uint32_t address, uint32_t size);
 ...
 * @return Status of execution as defined in @ref xtio_error_codes_t
 */
uint32_t xtio_flash_write(uint32_t address, uint8_t* data, uint32_t size);
...
/**
 * @brief Returns an ascii string of a cpu unique identifyer. 
 *
 * @return Number of bytes in flash
 */
uint32_t xtio_flash_get_unique_id(uint8_t* buffer, uint32_t max_size, uint32_t* size);
...
/**
 * @brief Configures UART module with the specified baudrate.
 *
 * @return Status of execution as defined in @ref xtio_error_codes_t
 */
int xtio_uart_set_baudrate(
...
// Typedef of function to send data
typedef int (* xtio_host_send_t)(uint8_t*, uint32_t, xtio_host_send_completed_t, void*);

// Typedef of function to initialize communication
typedef int (* xtio_serial_init_t)(xtProtocol*);
...
/**
 * Send a data buffer to host through debug interface
 *
 * @return Status of execution as defined in @ref xtio_error_codes_t
 */
int xtio_host_send_debug(
    uint8_t * buffer,                       ///< Pointer to buffer with data to send
    uint32_t length,                        ///< Length of data to send
    xtio_host_send_completed_t callback,    ///< Callback when completed
    void* callback_arg                      ///< Argument for callback
...
/**
 * Initiate debug serial communication
 *
 * @return Status of execution as defined in @ref xtio_error_codes_t
 */
int xtio_serial_com_debug_init(
	xtProtocol * xt_protocol       ///< Pointer to an xtProtocol object used for parsing communication
	);

/**
 * Check if we have a bootloader available.
 */
bool xt_has_bootloader(void);

/**
 * Return a string identifying what type of bootloader we have, or "" if we
 * don't.
 */
