...
def simple_xep_plot(device_name, record=False, baseband=True):
    FPS = 23
    directory = '.'
    reset(device_name)
    mc = pymoduleconnector.ModuleConnector(device_name)
    ...
    xep = mc.get_xep()
    # Set DAC range
    xep.x4driver_set_dac_min(850)
    xep.x4driver_set_dac_max(1200)

    # Set integration
    xep.x4driver_set_iterations(64)
    xep.x4driver_set_pulses_per_step(15)

    xep.x4driver_set_downconversion(int(baseband))
    # Start streaming of data
    xep.x4driver_set_frame_area(-0.3, 0.6)
    ...
    ax.set_ylim(0 if baseband else -0.1,0.1) #keep graph in frame (FIT TO YOUR DATA)
    frame = read_frame()
    if baseband:
        frame = abs(frame)
    line, = ax.plot(frame)
